...
class ApplicationTest(unittest.TestCase):
    ...
    def test_add(self):
        ...
        self.assertRaises(
            TypeError,
            self.calculator.add(self.raise_param[0], self.raise_param[1])
        )
    ...
    def test_divide(self):
        ...
        self.assertRaises(
            TypeError,
            self.calculator.divide(self.raise_param[0], self.raise_param[1])
        )

    def test_sqrt(self):
        for _, j in self.param_list:
            self.assertEqual(self.calculator.sqrt(j), math.sqrt(j))
        self.assertRaises(TypeError, self.calculator.sqrt(self.raise_param[1]))

    def test_exp(self):
        for _, j in self.param_list:
            self.assertEqual(self.calculator.exp(j), math.exp(j))
        self.assertRaises(TypeError, self.calculator.exp(self.raise_param[1]))
